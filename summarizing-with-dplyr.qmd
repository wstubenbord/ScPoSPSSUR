...
There isn't much of a difference between the two, so you can use whichever you prefer as long as you are consistent.[^summarizing-with-dplyr-4]
...
The pipe operator has a straightforward function: it combines a series of steps into a single command and it does this in a way that keeps your code legible. Whenever you see the pipe operator, you should read it as though it is saying, "And then \[do this\]" [@healy2019].
...
Take a quick look at the data. You can use `glimpse()`, another `dplyr` function, to get a sense of what's inside and you can inspect it visually using `view()`. Typing in `?gss_sm` (the original name of the data set from the package) will tell you what variables the data contains.[^summarizing-with-dplyr-6]
...
Now we have a new object containing our new tibble. If you inspect this new tibble and then decide that you don't need or want it anymore, you can always get rid of it using the `rm()` function.[^summarizing-with-dplyr-7]
