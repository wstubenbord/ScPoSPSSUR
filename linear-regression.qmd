...
```{r}
# The probs argument specifies the desired range of the percentiles returned and the increment.  In this case, we want to see deciles of income, so we use a 0.1 increment.  But, if we wanted to see quartiles, for example, we would use a 0.25 increment.
...
$\hat{y}_{i} = \hat{\beta}_{0} + \hat{\beta}_{1}x_1$
...
Where $educ_i$ is the level of education for a survey respondent, $\hat{\beta}_{0}$ is the y-intercept, $\hat{\beta}_{1}$ is the slope of the line (the estimated effect of education on hourly wage), and $\widehat{wage}_{i}$ is the predicted hourly wage for a given level of education.
