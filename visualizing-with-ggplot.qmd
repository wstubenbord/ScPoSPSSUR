...
![Florence Nightingale's (1858), "Diagram of the causes of mortality in the army in the East." This is a more effective form of pie chart where the perceptual relative size trade-off is negated by the amount of information conveyed and the seasonal nature of the data.](images/Nightengale_diagram.jpg)
...
That's maybe not the result we thought it was going to be. Instead of one smooth curve, as before, we now have we now have a different colored curve for each of the continents. We can also see in the key on the right-hand side that population size is affecting the width of the lines.
...
We're going to change the x-axis scale to a logarithmic scale, since the data appears to follow a logarithmic form, and then we're going to fix another scale, fix the labels, and add a title. Scales can be changed by adding functions from the `scale_` family to our plot. Like `geom_`, there are a variety of options depending on the need. In this case, we want a logarithmic scale for our x-axis in base 10 (a standard logarithm), so we use `scale_x_log10()`.
