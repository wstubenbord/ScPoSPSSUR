...
This chapter is split into two parts. We'll first continue where we left off in the last chapter, covering additional topics related to linear regressions. We'll then cover the implementation of a inferential method used to test relationships between categorical variables.
...
gss_no_na %>%
  ...
  chisq.test()
```
Easy enough!  We've obtained the chi-squared value (the test statistic) and the p-value.  The reported p-value is less than 0.05 and so we can therefore reject the null hypothesis.  There is evidence of an association marital status and reported levels of happiness in this data.
