...
<<<<<<< HEAD
At this point in the course, we have primarily been working with data from packages, which is generally convenient and straight forward. We install the package with the data we want, we load the package from our library, and then we access the data and begin summarizing it. The bad news is that the data we might want to use for a particular project often isn't contained in a package. The worse news is that we usually need to clean and transform the data in order to get it into a format that actually works for analysis.
=======
At this point in the course, we have primarily been working with data from packages, which is generally convenient and straight forward. We install the package with the data we want, we load the package from our library, and then we access the data and begin summarizing it. The bad news is that the data we want to use for a particular project oftentimes isn't contained in a package. The worse news is that we usually need to clean and transform the data in order to get it into a format that actually works for analysis.
>>>>>>> 62b1b27554b07dea49f49c85cd9cdb3a18401227
...
Up until now, we've been writing our code in plain-text files saved with a `.R` file extension (or what we have been calling R Script files) and we haven't needed to load data from other files or to save anything.

But, this isn't entirely true. We have actually been loading data from files saved on our computers, it just happens to be case that our use of packages and the `library()` function have concealed and greatly simplified the back-end interactions leading to data showing up in our RStudio environments. This is why, if you were to disable the Wi-Fi on your computer, you would still be able to access the `gss_sm` data from the `socviz` package.

As with any type of computer program, `R` operates within the file structure of your computer. It uses data, functions, and other compiled code which has been saved across a number of files (and file types) in different locations to run the program and to enable you to interact with the program. This is the case for everything from the apps on your smartphone to the operating system on your laptop. The reason we usually don't see the underlying files or code is because software developers realize that most people would rather not be bothered by these sorts of technical details, or worse, they might mess something up. We don't need to know how an engine works to drive a car, after all, and if we were to start tinkering around inside the engine bay we might break things.

For this chapter, we don't need to go deep into the code of `R` or `RStudio`. If you find yourself especially interested in how software works, you might be interested in taking a computer science course. All we need to know, instead, is how to get `R` to interact with files saved in different locations on our computers and how to organize them in a way that makes them easy to access.

## File Structures, File Paths, and the Working Directory

Your computer's operating system (Windows, MacOS, or Linux) organizes the files on your computer into folders. You might have created some folders on your own, renamed some, or stored files like photos, documents, and other items across them. There are usually default folders for files of a particular type. Pictures might go in a "Pictures" folder, for example, and documents in a "Documents" folder. Within a "Documents" folder, you might then have a folder for "Sciences Po" and another sub-folder for "SPSSUR." The way files are organized across folders and sub-folders on a computer's hard drive is usually referred to as your computer's **file structure**. Within a file structure, each file has what is called a **file path**, an address which tells you where the file is located. In Windows, they usually look something like this:
...
As an important aside, when you write file paths inside of `R`, you will generally want to write them in the MacOS format you see above (i.e., using forwards slashes, `/`) even if you are using a PC. This is because the backwards slash, `\`, is a special character in `R`.

Within the operating systems themselves, file paths are slightly easier to find in Windows than in MacOS, since you can get most of the way there by clicking in the address bar at the top of a Windows Explorer window (see below for an example).[^workflows-and-wrangling-1] This gives the folder path, which when followed by another slash, the file name, and the file extension, gives the file path. In MacOS, finding a file path requires a little more work (see [here](https://support.apple.com/en-mt/guide/mac-help/mchlp1774/mac#:~:text=On%20your%20Mac%2C%20click%20the,bottom%20of%20the%20Finder%20window.) for some guidance).[^workflows-and-wrangling-2]

[^workflows-and-wrangling-1]: Note that if you copy and paste a path from a Windows Explorer into `R`, you'll have to change the direction of the `\` to `/` or deal with the `\` issue in a different way (there are other ways, but hopefully you won't need to copy and paste paths anyways).

[^workflows-and-wrangling-2]: If you are using a Linux-based OS, you probably won't need an explanation of file structures and file paths, given the greater transparency of file paths and structures you regularly encounter.

![](images/Windows-file-path.PNG){width="598"}

When working with multiple files in `R`, as you will in more involved analysis projects, you will have to pay some attention to the file structure and file paths. When you load data from a file, for instance, `R` will need to know exactly where the file is located. If you are saving a graph, on the other hand, `R` will need to know where you want to save it.

<<<<<<< HEAD
`R` makes an educated guess as to where in the file structure you are working (or will be working) based on how you open `R`. This is called the **working directory** and you can identify where `R` has located it using the command below:
=======
`R` makes an educated guess as to where in the file structure you are working (or will be working) based on how you open `R`. This is called the **working directory** and you can identify where `R` has located using the command below:
>>>>>>> 62b1b27554b07dea49f49c85cd9cdb3a18401227
...
<<<<<<< HEAD
Sometimes the working directory doesn't quite match where you think you are working from and you may need to change it manually. We will try to avoid this, but if you need to, you can manually set the working directory using `setwd()`.

## The Problem with File Paths

As mentioned, you will generally have to provide a file path for any function that loads data into `R`. The problem with file structures and file paths, however, is that everyone is different. So, if we have a file saved in a specific location on our computer and we have some code that reads it in, how do we make sure that other people can use our code when they have the file saved in a different location on their computer?
=======
Sometimes the working directory doesn't quite match where you think you are working from and you may sometimes need to change it manually. We will try to avoid this, but if you need to, you can manually set the working directory using `setwd()`.

## The Problem with File Paths

As mentioned, you will generally have to provide a file path for any function that loads data into `R`. The problem with file structures and file paths, however, is that everyone is different. So, if we have file saved in a specific location on our computer and we have some code that reads it in, how do we make sure that other people can use our code when they have the file saved in a different location on their computer?
>>>>>>> 62b1b27554b07dea49f49c85cd9cdb3a18401227

To make this more concrete, let's say that I send you some code in an R Script along with a data file called `important_data.csv`. In the code which generates all sorts of nice figures and analysis using this data, I have a line that looks like this:

```         
read_csv("C:/Users/wcs26/Documents/Sciences Po/SPSSUR/important_data.csv")
```

This command tries to read in a data file, `important_data.csv`, located within `C:/`, the `Users` folder, the `wcs26` folder, and so on and so forth. Your code will attempt to read the file located at the end of this path, but as soon as it hits a folder it can't find on your computer, it will stop and produce an error:
...
<<<<<<< HEAD
This type of construction, in which every folder and sub-folder on the way to the destination is provided, is called an **absolute** file path. If any part of the address is incorrect, `R` won't be able to find the file and the file won't be read. So, what's the alternative?
=======
This type of file path construction, in which every folder and sub-folder on the way to the destination is provided, is called an **absolute** file path. If any part of the address is incorrect, `R` won't be able to find the file and the file won't be read. You won't be able to re-create all of my nice graphs and analysis as a result. So, what's the alternative?
>>>>>>> 62b1b27554b07dea49f49c85cd9cdb3a18401227
...
In this case, because we haven't provided the full address, `R` fills in the rest by guessing and, naturally, it guesses that the missing part of the address is the working directory (i.e., `C:/Users/wcs26/Documents/Sciences Po/SPSSUR/`) - which is where it believes you are working from. If `R` guessed correctly and the file is located in your working directory, then this will work perfectly and the data will be read correctly. If the file isn't located in the working directory, however, it will fail.

<<<<<<< HEAD
The trouble is that sometimes you will start your `R` session in one place and then use files in another place. Since `R` can only keep track of one working directory at a time (per session), it (or rather you) will get confused quickly and your relative file paths will also fail.
=======
The trouble is that sometimes you will start your `R` session in one place and then use files in another place. Since `R` can only keep track of one working directory at a time (per session), it (or rather you) will get confused quickly and even your relative file paths will fail.
>>>>>>> 62b1b27554b07dea49f49c85cd9cdb3a18401227

## R Projects and `here`

Fortunately, there is a better way. To avoid these working directory problems and the borderline criminal act of hard-coding absolute file paths in scripts, we're going to use two solutions that will help keep our working directory and file paths straight.

### R Projects

The first is R Projects, a tool to organize your files in R Studio. When you create an R Project file (`.Rproj`), R Studio creates a new folder on your computer which is made to store all of the associated files you may have for a project (e.g., your data, your code, and any outputs). Every time you open that R Project file, R Studio then opens a new working session with a working directory correctly set to the location of your project file. All of the files will be right where you need them.

Let's create one for today's classwork. In R Studio, use the navigation bar at the top of your screen to go to File \> New Project \> New Directory \> New Project. Then, in the provided prompt, type in a project name that matches our course naming standards (e.g., "Stubenbord_Wesley_Session 5 Classwork"). Create this project as a sub-directory of your SPSSUR course folder (wherever this may be and whatever it may be called on your computer). Once this is done, this is where your projects files will live - a permanent home just for them. You can verify that the new project folder has been created by navigating to it in your computer's file structure browser (e.g., Windows Explorer in Windows).

![](images/Creating-R-Project.PNG){width="409"}

<<<<<<< HEAD
After you create a new project, you'll find yourself in a clean R Studio session with only a console window open. The folder in your computer system will contain only the new `.Rproj` file and a sub-folder with some saved settings stored for your project. This `.Rproj` file is how you will access your project from now on. Instead of opening a .R script to access your code, you will always want to open your `.Rproj` file first and then the .R script. Don't put anything in this new folder other than files directly associated with the project you are working on. In this case, that means this folder is only intended to store files associated with today's classwork.
=======
After you create a new project, you'll find yourself in a clean R Studio session with only a console window open. The folder in your computer system will contain only the new `.Rproj` file and a sub-folder with some saved settings stored for your project. This .Rproject file is how you will access your project from now on. Instead of opening a .R script to access your code, you will always want to open your .Rproject file and then the .R script. Don't put anything in this new folder other than files directly associated with the project you are working on. In this case, that means this folder is only intended to store files associated with today's classwork.
>>>>>>> 62b1b27554b07dea49f49c85cd9cdb3a18401227
...
=======
In any case, the most common format you are likely to find data stored in is a CSV file, which stands for comma separated values. CSVs are particularly appealing for data storage because they are lightweight and they simple. They don't contain any extra formatting - all they consist of is plain-text data in rows (separated by new lines) and columns (separated by commas). A CSV file could look as simple as this, for example:
>>>>>>> 62b1b27554b07dea49f49c85cd9cdb3a18401227
...
<<<<<<< HEAD
When you open a CSV file in software like Microsoft Excel, it is usually automatically parsed into different columns, based on the position of the commas, and into different rows, based on the line breaks.[^workflows-and-wrangling-3] Excel will automatically recognize, for example, that name is a column containing two values, "Bill Gates" in one row and "Warren Buffett" in another. CSVs don't always use commas to separate their values - sometimes (especially in Europe), they use semi-colons. The character (e.g., `,` or `;`) that separates values in a data file is called a **delimiter**.
=======
When you open a CSV file in software like Microsoft Excel, it is usually automatically parsed into different columns, based on the position of the commas, and different rows, based on the line breaks.[^workflows-and-wrangling-3] Excel will automatically recognize, for example, that name is a column containing two values, "Bill Gates" in one row and "Warren Buffett" in another. CSVs don't always use commas to separate their values - sometimes (especially in Europe), they use semi-colons. The character (e.g., `,` or `;`) that separates values in a data file is called a **delimiter**.
>>>>>>> 62b1b27554b07dea49f49c85cd9cdb3a18401227

[^workflows-and-wrangling-3]: **Parsing** refers to how software reads the values.

![](images/excel-parsing.PNG){width="607"}

## Loading files from CSVs

Thankfully, loading CSV files is not very difficult in `R`. `R` has a base function for loading CSVs, `read.csv()`, but there is a better version called `read_csv()` which comes from the `readr` package. `readr` is also located in the `tidyverse`, but must also be loaded separately, just like the `scales` package in the previous chapter.[^workflows-and-wrangling-4]

[^workflows-and-wrangling-4]: As before, rather than load the entire `readr` package via `library()`, you can also use the `::` syntax, as in `readr::read_csv()`.
